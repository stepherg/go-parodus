...
import (
	...
	libparodus "github.com/stepherg/go-parodus/client"
...
func (p *Parodus) msgHandler(wrpBus chan wrp.Message) {
	...
	for {
		...
		case msg := <-wrpBus:
			...
			default:
				logging.Error(p.logger).Log(logging.MessageKey(), "Unexpected WRP Message. Please file an issue at github.com/stepherg/go-parodus/issues", "wrp", msg)
			}
		}
	}
}
